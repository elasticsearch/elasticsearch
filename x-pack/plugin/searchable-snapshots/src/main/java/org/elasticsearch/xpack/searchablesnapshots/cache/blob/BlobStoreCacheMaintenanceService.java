...
public class BlobStoreCacheMaintenanceService implements ClusterStateListener {
    ...
    public void clusterChanged(ClusterChangedEvent event) {
        ...
        if (systemIndexPrimaryShardActiveAndAssignedToLocalNode(state)) {
    ...
    private boolean systemIndexPrimaryShardActiveAndAssignedToLocalNode(final ClusterState state) {
        for (String matchingSystemIndex : systemIndexDescriptor.getMatchingIndices(state.metadata())) {
            final IndexMetadata indexMetadata = state.metadata().index(matchingSystemIndex);
