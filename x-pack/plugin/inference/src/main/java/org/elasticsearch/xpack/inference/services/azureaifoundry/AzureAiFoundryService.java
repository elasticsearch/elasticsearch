...
import org.elasticsearch.xpack.inference.external.action.azureaifoundry.AzureAiFoundryActionCreator;
...
import static org.elasticsearch.xpack.inference.services.azureaifoundry.completion.AzureAiFoundryChatCompletionTaskSettings.DEFAULT_MAX_TOKENS;
import static org.elasticsearch.xpack.inference.services.openai.OpenAiServiceFields.EMBEDDING_MAX_BATCH_SIZE;

public class AzureAiFoundryService extends SenderService {

    static final String NAME = "azureaifoundry";

    private static final String SERVICE_NAME = "Azure AI Foundry";
    private static final EnumSet<TaskType> supportedTaskTypes = EnumSet.of(TaskType.TEXT_EMBEDDING, TaskType.COMPLETION);

    public AzureAiFoundryService(HttpRequestSender.Factory factory, ServiceComponents serviceComponents) {
    ...
    protected void doInfer(
        ...
        var actionCreator = new AzureAiFoundryActionCreator(getSender(), getServiceComponents());

        if (model instanceof AzureAiFoundryModel baseAzureAiFoundryModel) {
            var action = baseAzureAiFoundryModel.accept(actionCreator, taskSettings);
    ...
    protected void doChunkedInfer(
        ...
        if (model instanceof AzureAiFoundryModel baseAzureAiFoundryModel) {
            ...
            List<EmbeddingRequestChunker.BatchRequestAndListener> batchedRequests = new EmbeddingRequestChunker(
                ...
                baseAzureAiFoundryModel.getConfigurations().getChunkingSettings()
    ...
    public void parseRequestConfig(
        ...
        try {
            ...
            AzureAiFoundryModel model = createModel(
                inferenceEntityId,
                taskType,
                serviceSettingsMap,
                taskSettingsMap,
    ...
    @Override
    public AzureAiFoundryModel parsePersistedConfigWithSecrets(
        String inferenceEntityId,
        TaskType taskType,
        Map<String, Object> config,
    ...
    private static AzureAiFoundryModel createModel(
        String inferenceEntityId,
        TaskType taskType,
        ...
        if (taskType == TaskType.TEXT_EMBEDDING) {
            var embeddingsModel = new AzureAiFoundryEmbeddingsModel(
                inferenceEntityId,
                taskType,
                NAME,
                serviceSettings,
        ...
        if (taskType == TaskType.COMPLETION) {
            var completionModel = new AzureAiFoundryChatCompletionModel(
                inferenceEntityId,
                taskType,
                NAME,
                serviceSettings,
    ...
    private AzureAiFoundryModel createModelFromPersistent(
        String inferenceEntityId,
        TaskType taskType,
        Map<String, Object> serviceSettings,
        Map<String, Object> taskSettings,
    ...
    public Model updateModelWithEmbeddingDetails(Model model, int embeddingSize) {
        if (model instanceof AzureAiFoundryEmbeddingsModel embeddingsModel) {
            var serviceSettings = embeddingsModel.getServiceSettings();
            var similarityFromModel = serviceSettings.similarity();
            var similarityToUse = similarityFromModel == null ? SimilarityMeasure.DOT_PRODUCT : similarityFromModel;

            var updatedServiceSettings = new AzureAiFoundryEmbeddingsServiceSettings(
            ...
            return new AzureAiFoundryEmbeddingsModel(embeddingsModel, updatedServiceSettings);
        } else {
    ...
    public Model updateModelWithChatCompletionDetails(Model model) {
        if (model instanceof AzureAiFoundryChatCompletionModel chatCompletionModel) {
            ...
            var updatedTaskSettings = new AzureAiFoundryChatCompletionTaskSettings(
            ...
            return new AzureAiFoundryChatCompletionModel(chatCompletionModel, updatedTaskSettings);
        } else {
            throw ServiceUtils.invalidModelTypeForUpdateModelWithChatCompletionDetails(model.getClass());
        }
    }

    private static void checkDeploymentTypeAndModel(AzureAiFoundryDeploymentType deploymentType, String model) {

        if (deploymentType == AzureAiFoundryDeploymentType.AZURE_AI_MODEL_INFERENCE_SERVICE && model == null) {
        ...
        private static final LazyInitializable<InferenceServiceConfiguration, RuntimeException> configuration = new LazyInitializable<>(
            () -> {
                ...
                configurationMap.put(
                    TARGET_FIELD,
                    new SettingsConfiguration.Builder().setDescription("The target URL of your Azure AI Foundry model deployment.")
                        .setLabel("Target")
                        .setRequired(true)
                        .setSensitive(false)
                        .setUpdatable(false)
